...
export default function BurndownChart() {
  const [sprints, setSprints] = useState<SprintData[]>(() => {
    ...
    const savedSprints = typeof window !== "undefined" ? localStorage.getItem("sprintData") : null
    if (savedSprints) {
      try {
        return JSON.parse(savedSprints)
      } catch (e) {
  ...
  const updateChartData = () => {
    ...
    const cumulativeAdded = {
      ...
      wednesday: sprint.dailyAdded.monday + sprint.dailyAdded.tuesday + sprint.dailyAdded.wednesday,
      thursday:
        sprint.dailyAdded.monday + sprint.dailyAdded.tuesday + sprint.dailyAdded.wednesday + sprint.dailyAdded.thursday,
      friday:
    ...
    const cumulativeCompleted = {
      ...
      wednesday: sprint.dailyCompleted.monday + sprint.dailyCompleted.tuesday + sprint.dailyCompleted.wednesday,
      thursday:
  ...
  const updateSprintData = (updatedSprint: SprintData) => {
    ...
    const remaining = {
      monday: updatedSprint.startPoint + updatedSprint.dailyAdded.monday - updatedSprint.dailyCompleted.monday,
      tuesday:
        updatedSprint.startPoint +
        updatedSprint.dailyAdded.monday +
        updatedSprint.dailyAdded.tuesday -
        (updatedSprint.dailyCompleted.monday + updatedSprint.dailyCompleted.tuesday),
      wednesday:
        updatedSprint.startPoint +
        updatedSprint.dailyAdded.monday +
        updatedSprint.dailyAdded.tuesday +
    ...
    setSprints(sprints.map((sprint) => (sprint.id === finalUpdatedSprint.id ? finalUpdatedSprint : sprint)))
  }

  // Add a function to update sprint number
  const updateSprintNumber = (id: number, newNumber: number) => {
    setSprints(sprints.map((sprint) => (sprint.id === id ? { ...sprint, sprintNumber: newNumber } : sprint)))
  }
  ...
  const deleteSprint = (id: number) => {
    ...
    if (selectedSprint === id && sprints.length > 1) {
      ...
      if (remainingSprints.length > 0) {
        ...
        const latestSprint = [...remainingSprints].sort((a, b) => b.sprintNumber - a.sprintNumber)[0]
    ...
    try {
      const newSprints = importedData.map((item, index) => {
        ...
        const totalAdded =
          ...
          dailyAdded.monday + dailyAdded.tuesday + dailyAdded.wednesday + dailyAdded.thursday + dailyAdded.friday
      ...
      {showDashboard ? (
        <div className="dashboard-section">
          <Card>
            ...
            <CardContent>
              {sprints.length > 0 ? (
                <DashboardOverview sprints={sprints} onSelectSprint={handleSelectSprint} />
              ) : (
                <div className="text-center py-8">
                  <p className="text-muted-foreground">No sprint data available. Add a sprint to get started.</p>
                </div>
              )}
            </CardContent>
          </Card>
          ...
          <div className="lg:order-2">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Visual representation of sprint progress</CardDescription>
              </CardHeader>
                      ...
                      <Tooltip
                        content={({ active, payload, label }) => {
                          if (active && payload && payload.length) {
                            ...
                            const ideal = payload.find((p) => p.dataKey === "ideal")?.value || 0
                              ...
                              <div className="bg-popover border border-border rounded-md shadow-md p-3 text-popover-foreground">
                                ...
                                <div className="space-y-1 text-sm">
                                  ...
                                  <div className="grid grid-cols-2 gap-x-4">
                                    ...
                                    <span className="font-medium text-right">{ideal}</span>
                                  </div>
                                  {label !== "Start" && (
                                    <>
                                      <div className="grid grid-cols-2 gap-x-4">
